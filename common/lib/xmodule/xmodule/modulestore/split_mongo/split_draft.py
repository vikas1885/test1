...
class DraftVersioningModuleStore(SplitMongoModuleStore, ModuleStoreDraftAndPublished):
    ...
    def _get_orphan_data(self, draft_key, published_key):
        """
        Get requisite information about draft and published orphans
        to delete them appropriately.
        """
        draft_orphans = self.get_orphans(draft_key)
        published_orphans = self.get_orphans(published_key)
        ...
        published_block_ids = [
            block.id for block in self._lookup_course(published_key).structure['blocks']
        ]

        draft_block_ids = [
            block.id for block in self._lookup_course(draft_key).structure['blocks']
        ]

        orphans = draft_orphans + published_orphans

        return {
            "draft_orphan_ids": draft_orphan_ids,
            "published_orphan_ids": published_orphan_ids,
            "draft_block_ids": draft_block_ids,
            "published_block_ids": published_block_ids,
            "orphans": orphans,
        }
    ...
    def delete_orphans(self, course_key, user_id, **kwargs):
        ...
        orphan_data = self._get_orphan_data(
            draft_preferred_course_key, published_course_key
        )
        orphans = orphan_data["orphans"]
        draft_orphan_ids = orphan_data["draft_orphan_ids"]
        published_orphan_ids = orphan_data["published_orphan_ids"]
        draft_block_ids = orphan_data["draft_block_ids"]
        published_block_ids = orphan_data["published_block_ids"]
        ...
        while orphans:
            ...
            if orphan.block_id in (set(draft_orphan_ids) & set(published_orphan_ids)):
                ...
                self.delete_item(
                    orphan, user_id, revision=ModuleStoreEnum.RevisionOption.all
                )
                ...
                if orphan.block_id not in published_block_ids:
                    ...
                    published_location = orphan.for_branch(
                    ...
                    draft_would_be_parent = published_parent.for_branch(
                ...
                if orphan.block_id not in draft_block_ids:
                    ...
                    if draft_parent.category in DIRECT_ONLY_CATEGORIES:
                        self.revert_single_block_to_published(
                            draft_parent, user_id, force=True
                        )
                        self.delete_item(
                            orphan,
                            user_id,
